...
{
	int id;
	if (MyArknightsScriptScript::AllSoundSource.size() == 0) {
		id = 0;
	}
	else {
		id = MyArknightsScriptScript::AllSoundSource.rbegin()->first;
	}
...
int MASS::GenerateActor(const wchar_t * name)
{
	...
	a->name = name;
	int id;
	if (MyArknightsScriptScript::AllActor.size() == 0) {
		id = 0;
	}
	else {
		id = MyArknightsScriptScript::AllActor.rbegin()->first;
	}
